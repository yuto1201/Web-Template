...
async function readContainedContract(root, contract) {
  ...
  if (!isContainedPath(canonicalRoot, contractRoot)) {
    throw new Error("Agent contract directory must stay inside the repository root.");
  }
  const contractPath = await realpath(path.resolve(canonicalRoot, contract));
  if (!isContainedPath(contractRoot, contractPath)) {
