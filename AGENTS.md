...
- Routine GitHub collaboration uses `npm run github:workflow` under protected-main `config/github-workflow.json`: approved proposal intake may precede an Issue; branch/PR writes bind a real open Issue and exact Head. Its common-directory journal and create-only remote claim refs do not require source evidence commits. It cannot merge, delete refs, activate providers, or use Cursor Cloud; see `docs/github-workflow.md`.
