...
5. A later Issue may submit a strict `cloudflare.upsert_dns` request whose frozen constraints contain the zone ID, hostname, record type, target, `proxied: false`, and the Vercel routing source with recommendation digest. No Cloudflare production provider client ships in this release, so the registered contract currently fails closed. The future client must obtain its own live preflight, claim-time, and postflight observations and execute only that exact upsert with verified provider idempotency.
...
The desired content is accepted only from a `source: vercel-api` observation bound to the canonical Vercel team, project, and hostname. A future production client must obtain the value from the live provider response through the same authenticated surface; caller-authored JSON is never identity or target provenance. CNAME targets must match Vercel's documented `*.vercel-dns[-N].com` form; other targets fail closed.
